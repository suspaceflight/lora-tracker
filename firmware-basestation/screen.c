...
static void screen_write_i2c_start(uint32_t i2c, uint8_t i2c_addr, uint8_t size);
static void screen_write_i2c_next_byte(uint32_t i2c, uint8_t byte);

static volatile uint8_t op_in_progress = 0;
...
void screen_write_text(char *text, uint8_t position)
{
	...
	op_in_progress=1;
	screen_write_i2c_start(I2C1,0x7C,len*2);
	screen_write_i2c_next_byte(I2C1, S_CMDSEND | S_MULTIPLE);
	screen_write_i2c_next_byte(I2C1, S_SET_DDRAM | position);
	...
	{
		screen_write_i2c_next_byte(I2C1, S_DATASEND | S_MULTIPLE);
		screen_write_i2c_next_byte(I2C1, text[i]);
	}
	op_in_progress=0;
}
